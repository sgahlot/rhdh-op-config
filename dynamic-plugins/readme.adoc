...
You can create dynamic plugin support for a third-party frontend plugin with or without starter files. If you want to use starter files, copy the starter files from {uri-dynamic-plugin-fe-starter-files}[Dynamic plugin wrapper for frontend - starter files] to an empty directory. If you do not want to use starter files, complete the following steps:
...
You can create dynamic plugin support for a third-party frontend plugin with or without starter files. If you want to use starter files, copy the starter files from {uri-dynamic-plugin-be-starter-files}[Dynamic plugin wrapper for backend - starter files] to an empty directory. If you do not want to use starter files, complete the following steps:
...
----
<1> add `@janus-idp/cli` dependency, which provides a new, required, `export-dynamic-plugin` command. _Version `1.7.7` of the janus-cli is in tech-preview at the moment. If you're unsure of using this version then use an older version, e.g. `1.4.7`_
<2> add `dist` to the package files list
<3> add `export-dynamic` to scripts. This will also embedd the dependencies as well as the third party package. _If using another version of janus-cli other than `1.7.7`, remove the `--embed-as-dependencies` argument from `export-dynamic` script_
              ...
              <NAME_OF_YOUR_DYNAMIC_PLUGIN>:                                                  # <.>
                dynamicRoutes:
                  - importName: <THIRD_PARTY_COMPONENT>                                       # <.>
                    ...
                    path: /<UNIQUE_PATH>                                                      # <.>
----
<1> <WrapperPluginName>-<WrapperPluginVersion>.tgz - file name that was created with `npm pack` command (prefixed with `http://plugin-registry:8080` - this is where our plugin-registry app is running)
